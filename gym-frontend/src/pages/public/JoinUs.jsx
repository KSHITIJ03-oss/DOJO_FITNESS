/**
 * Join Us page - Membership inquiry form.
 * Public page - no authentication required.
 */
import { useState } from 'react';
import { submitJoinRequest } from '../../api/leads';
...
const JoinUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setFormData({ name: '', mobile: '', email: '', interested_plan: '', message: '' });
    } catch (error) {
      ...
      <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl font-black text-white mb-6">
            JOIN THE <span className="text-primary-400">ELITE</span>
          </h1>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Become a member and unlock unlimited access to world-class facilities.
          </p>
        </div>
      </section>
          ...
          <div className="bg-gray-900 rounded-2xl p-8 md:p-12 border border-gray-800 shadow-2xl">
            {success ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-8">Thank you! Our team will contact you soon with membership details.</p>
                <CTAButton onClick={() => setSuccess(false)} variant="primary" size="md">Submit Another Request</CTAButton>
              </div>
            ) : (
              <form onSubmit={handleSubmit}>
                <h2 className="text-2xl font-bold text-white mb-8">Request Membership Info</h2>
                {errors.submit && (
                ...
                <div className="mb-6">
                  <label className="block text-sm font-semibold text-gray-300 mb-2">Interested Plan</label>
                  <select name="interested_plan" value={formData.interested_plan} onChange={handleChange} disabled={loading} className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50">
                ...
                <CTAButton type="submit" variant="primary" size="lg" className="w-full" disabled={loading}>
                  {loading ? 'Submitting...' : 'Request Membership Info'}
                </CTAButton>
              </form>
            )}
          </div>
